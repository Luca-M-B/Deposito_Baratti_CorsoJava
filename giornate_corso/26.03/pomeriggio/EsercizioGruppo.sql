...
CREATE TABLE
    Medico (
        ...
        CONSTRAINT fk_medico_spec FOREIGN KEY (id_specializzazione) REFERENCES Specializzazione (id_specializzazione) ON DELETE RESTRICT,
...
CREATE TABLE
    Appuntamento (
        ...
        CONSTRAINT fk_app_paziente FOREIGN KEY (id_paziente) REFERENCES Paziente (id_paziente) ON DELETE CASCADE,
        CONSTRAINT fk_app_medico FOREIGN KEY (id_medico) REFERENCES Medico (id_medico) ON DELETE RESTRICT,
...
CREATE TABLE
    Prescrizione (
        ...
        CONSTRAINT fk_pres_app FOREIGN KEY (id_appuntamento) REFERENCES Appuntamento (id_appuntamento) ON DELETE CASCADE
    );
...
CREATE TABLE
    Ricovero (
        ...
        CONSTRAINT fk_ric_paziente FOREIGN KEY (id_paziente) REFERENCES Paziente (id_paziente) ON DELETE CASCADE,
        CONSTRAINT fk_ric_reparto FOREIGN KEY (id_reparto) REFERENCES Reparto (id_reparto) ON DELETE RESTRICT,
        CONSTRAINT chk_dimissione CHECK (
            data_dimissione IS NULL
            OR data_dimissione > data_ingresso
        )
    );
...
CREATE TABLE
    Pagamento (
        ...
        CONSTRAINT fk_pag_app FOREIGN KEY (id_appuntamento) REFERENCES Appuntamento (id_appuntamento) ON DELETE CASCADE,
...
-- Prescrizioni (15)
INSERT INTO
    Prescrizione (id_appuntamento, descrizione, data_prescrizione)
VALUES
    (
        1,
        'Cardioaspirina 100mg 1 cp/die. Controllo ECG fra 3 mesi.',
        '2024-01-10'
    ),
    (
        3,
        'Riposo assoluto per 2 settimane. Antinfiammatori se necessario.',
        '2024-02-08'
    ),
    (
        6,
        'Monitoraggio pressione quotidiano. Beta-bloccante 50mg.',
        '2024-01-22'
    ),
    (
        10,
        'RMN cerebrale urgente. Levetiracetam 500mg x2/die.',
        '2024-02-08'
    ),
    (
        12,
        'Chemioterapia ciclo 3. Antiemetico pre-trattamento.',
        '2024-08-22'
    ),
    (
        13,
        'Radiografia anca dx. Antidolorifico FANS.',
        '2024-01-30'
    ),
    (
        15,
        'Crema cortisonica 1%. Evitare esposizione solare diretta.',
        '2024-07-19'
    ),
    (
        17,
        'Vaccino influenzale. Paracetamolo al bisogno.',
        '2024-03-25'
    ),
    (
        19,
        'Fisioterapia 10 sedute. Ibuprofene 400mg se dolore acuto.',
        '2024-02-20'
    ),
    (
        20,
        'Ecografia addome. Dieta iposodica. Omeprazolo 20mg/die.',
        '2024-05-08'
    ),
    (
        22,
        'Controllo glicemia. Metformina 500mg x2/die.',
        '2024-06-10'
    ),
    (
        24,
        'Holter cardiaco 24h. Bisoprololo 5mg.',
        '2024-08-20'
    ),
    (
        26,
        'PET scan. Secondo ciclo chemioterapico.',
        '2024-09-28'
    ),
    (
        28,
        'Elettromiografia. Pregabalin 75mg x2/die.',
        '2024-10-22'
    ),
    (
        29,
        'Ecografia ginecologica. Acido folico 400mcg/die.',
        '2024-11-01'
    );
...
SELECT
    ...
    COUNT(Appuntamento.id_appuntamento) AS Numero_Appuntamenti
FROM
    Medico
...
SELECT
    ...
    Paziente.nome AS Nome_Paziente,
    COUNT(Appuntamento.id_appuntamento) AS Numero_Appuntamenti
FROM
    Paziente
...
SELECT
    Medico.id_medico,
    Medico.cognome AS Medico,
    Medico.nome AS Nome,
    SUM(Pagamento.importo) AS Incasso_Totale
FROM
    Medico
    JOIN Appuntamento a ON Medico.id_medico = Appuntamento.id_medico
...
SELECT
    ...
    Medico.nome,
    Medico.cognome,
    Specializzazione.nome AS specializzazione,
    Medico.stipendio
FROM
    Medico
...
WHERE
    Medico.stipendio = (
        SELECT
            MAX(Medico.stipendio)
        FROM
            Medico
        WHERE
            Medico.id_specializzazione = Specializzazione.id_specializzazione
    );
...
SELECT
    ...
    Paziente.nome,
    Paziente.cognome,
    SUM(Pagamento.importo) AS totale_speso
FROM
    Paziente
    JOIN Appuntamento ON Paziente.id_paziente = Appuntamento.id_paziente
    JOIN Pagamento ON Appuntamento.id_appuntamento = Pagamento.id_appuntamento
...
HAVING
    totale_speso = (
        ...
        FROM
            (
                SELECT
                    SUM(Pagamento.importo) AS totale
                FROM
                    Appuntamento
                    JOIN Pagamento ON Appuntamento.id_appuntamento = Pagamento.id_appuntamento
                GROUP BY
                    Appuntamento.id_paziente
            ) t
    );
 ...
 */
SELECT
    Reparto.nome AS reparto,
    COUNT(Ricovero.id_ricovero) AS ricoverati_attuali,
    Reparto.numero_posti,
    (COUNT(Ricovero.id_ricovero) * 100.0) / Reparto.numero_posti AS percentuale_occupazione
FROM
    Reparto
    LEFT JOIN Ricovero ON Reparto.id_reparto = Ricovero.id_reparto
...
WHERE
    NOT EXISTS (
        ...
        WHERE
            ...
            AND Appuntamento.data_appuntamento >= NOW () - INTERVAL 30 DAY
    );
...
FROM
    Specializzazione
    JOIN Medico ON Medico.id_specializzazione = Specializzazione.id_specializzazione
    JOIN Appuntamento a ON Appuntamento.id_medico = Medico.id_medico
    JOIN Pagamento ON Pagamento.id_appuntamento = Appuntamento.id_appuntamento
GROUP BY
    Specializzazione.id_specializzazione,
    Specializzazione.nome
HAVING
    SUM(Pagamento.importo) = (
        SELECT
            MAX(incasso_per_spec)
        FROM
            (
                SELECT
                    SUM(Pagamento.importo) AS incasso_per_spec
                FROM
                    Specializzazione
                    JOIN Medico ON Medico.id_specializzazione = Specializzazione.id_specializzazione
                    JOIN Appuntamento ON Appuntamento.id_medico = Medico.id_medico
                    JOIN Pagamento ON Pagamento.id_appuntamento = Appuntamento.id_appuntamento
                GROUP BY
                    Specializzazione.id_specializzazione
            ) AS incassi
...
SELECT
    ...
    incasso_precedente,
    ROUND(
        (incasso_totale - incasso_precedente) / incasso_precedente * 100,
        2
    ) AS variazione_percentuale
FROM
    (
        SELECT
            DATE_FORMAT (Pagamento.data_pagamento, '%Y-%m') AS mese,
            SUM(Pagamento.importo) AS incasso_totale,
            LAG (SUM(Pagamento.importo)) OVER (
                ORDER BY
                    DATE_FORMAT (Pagamento.data_pagamento, '%Y-%m')
...
/*
QUERY10
Mostrare i pazienti che hanno avuto almeno 2 ricoveri e hanno effettuato almeno 5 appuntamenti e hanno speso sopra la media dei pazienti
 */
SELECT
    Paziente.id_paziente,
    Paziente.nome,
...
WHERE -- almeno 2 ricoveri
    (
        ...
        FROM
            Ricovero
        WHERE
            Ricovero.id_paziente = Paziente.id_paziente
    ) >= 2
    AND -- almeno 5 appuntamenti
    (
        SELECT
            COUNT(*)
        FROM
    ...
    AND -- spesa sopra la media
    (
        SELECT
            COALESCE(SUM(Pagamento.importo), 0)
        FROM
            Appuntamento
            LEFT JOIN Pagamento ON Appuntamento.id_appuntamento = Pagamento.id_appuntamento
        WHERE
            Appuntamento.id_paziente = Pagamento.id_paziente
    ) > (
        -- media della spesa per paziente
        SELECT
            AVG(totale_speso)
        FROM
            (
                SELECT
                    Appuntamento.id_paziente,
                    SUM(pg2.importo) AS totale_speso
                FROM
                    Appuntamento
                    LEFT JOIN Pagamento ON Appuntamento.id_appuntamento = Pagamento.id_appuntamento
                GROUP BY
